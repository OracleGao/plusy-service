...
public class DataDao {
	...
	@Value("${plusy.store.path.data}")
	private File storePath;
	
	@Value("${plusy.store.limit}")
	...
	public void putSave(String spider, List<DataBean> datas) throws IOException {
		if (datas.size() > storeLimit) {
			datas = datas.subList(0, storeLimit);
		}
		cache.put(spider, datas);
		save(spider, datas);
	}
	...
	public void save(String spider) throws IOException {
		List<DataBean> datas = get(spider);
		save(spider, datas);
	}
	
	private void save(String spider, List<DataBean> datas) throws IOException {
